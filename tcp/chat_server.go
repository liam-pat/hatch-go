...
func main() {
	...
	if err != nil {
		fmt.Println("init server listener err", err)
		return
	}
	// defer the listener close
	...
	for {
		...
		if err1 != nil {
			fmt.Println("accept client err", err)
			return
		}
		go handleCon(con)
	}
...
func handleCon(conn net.Conn) {
	...
	// init one client, add it to online list
